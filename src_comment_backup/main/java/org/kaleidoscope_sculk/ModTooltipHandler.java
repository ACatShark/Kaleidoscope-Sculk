...
public class ModTooltipHandler {
    ...
    public static void onItemTooltip(ItemTooltipEvent event) {
        ...
        // 幽匿炖菜方块（介绍「maxim」由森罗厨房渲染，这里只在其下方追加 buff 效果）
        if (itemStack.getItem() == ModItems.SCULK_STEW_BLOCK_ITEM.get()) {
            event.getToolTip().add(Component.translatable("item.kaleidoscope_sculk.warmth")
        ...
        // 幽匿炖鸡煲（介绍「maxim」由森罗厨房渲染，这里只在其下方追加 buff 效果）
        if (itemStack.getItem() == ModItems.SCULK_CHICKEN_STEW_BLOCK_ITEM.get()) {
            event.getToolTip().add(Component.translatable("item.kaleidoscope_sculk.warmth_2min")
                    .withStyle(ChatFormatting.BLUE));
        }
        ...
        // 古城风味刺身（介绍「maxim」由森罗厨房渲染，这里只在其下方追加芥末效果）
        if (itemStack.getItem() == ModItems.ANCIENT_CITY_STYLE_SASHIMI.get()) {
            event.getToolTip().add(mustardTooltip());
        }
    }

    /**
     * 读取森罗厨房的芥末效果生成「名字 (时长)」，和游戏内药水提示一致（cookery 为硬依赖，必存在）。
     */
    private static Component mustardTooltip() {
        Holder<MobEffect> mustard = ModBlocks.getMustardEffect();
        MobEffect effect = mustard.value();
        MobEffectInstance instance = new MobEffectInstance(mustard, ModBlocks.MUSTARD_DURATION, 0);

        return Component.translatable("potion.withDuration",
