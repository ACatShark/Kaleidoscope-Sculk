...
/**
 * 魂幡数据：类型名、已存储经验点数、朝向（取自旗帜的 ROTATION）
 */
public record SoulSailData(String type, int storedXp, int rotation) {
    ...
    public static final StreamCodec<ByteBuf, SoulSailData> STREAM_CODEC = StreamCodec.composite(
            ByteBufCodecs.STRING_UTF8, SoulSailData::type,
            ByteBufCodecs.INT, SoulSailData::storedXp,
            ByteBufCodecs.INT, SoulSailData::rotation,
            SoulSailData::new
    );
